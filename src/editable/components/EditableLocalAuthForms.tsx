...
const inputClass = 'h-[3.35rem] border border-[color:rgba(138,95,65,0.24)] bg-[#fffdf8] px-4 text-base text-black outline-none transition placeholder:text-black/35 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-[3.35rem] items-center justify-center rounded-[0.35rem] bg-[var(--slot4-dark-bg)] px-6 text-xs uppercase tracking-[0.2em] text-white transition hover:bg-[var(--slot4-accent-fill)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`border px-4 py-3 text-sm ${status === 'success' ? 'border-emerald-800 bg-emerald-50 text-emerald-800' : 'border-red-700 bg-red-50 text-red-700'}`}>{message}</p> : null}
